...
describe('BORROWER', function() {
  ...
  it('Go to create loan', function() {
    ...
    cy.get('#btn-create-loan').click({ force: true });

    cy.get('#input-amount').should('have.length', 1);
    cy.wait(300);
    cy.get('#input-amount').matchImageSnapshot('amount', { force: true });
    cy.get('body').matchImageSnapshot('content_borrower', { force: true });
    cy.get('#input-amount')
      .clear()
      .type(10);
