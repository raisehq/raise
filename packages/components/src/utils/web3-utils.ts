...
export const isChecksumAddress = address => {
  ...
  for (let i = 0; i < 40; i += 1) {
    ...
    if (
      (parseInt(addressHash[i], 16) > 7 && notAddress[i].toUpperCase() !== notAddress[i]) ||
      (parseInt(addressHash[i], 16) <= 7 && notAddress[i].toLowerCase() !== notAddress[i])
    ) {
      return false;
    }
  }
...
export async function getTokenBalance(tokenAddress: any, address: any, library: any) {
