...
const getDesiredTime = (seconds, type?) => match(seconds,
  s => s >= secondUnits.month, s => `${roundedTime(s, secondUnits.month)} months`,
  ...
  ANY, () => type === 'loan' ? 'Expired' : 'Auction ended');

const defaultZero = numeral(0).format();
const calculateFromWei = number => number ? numeral(fromWei(number.toString(), 'ether')).format(numeralFormat) : defaultZero
...
const useCal = auction => {
  ...
  useAsyncEffect(async () => {
    ...
    setCalcs({ times: { loanTerm, auctionTimeLeft, loanTermLeft }, borrowerDebt, interest, maxAmount, netBalance, operatorFee, principal, systemFees, maxSystemFees });
  }, []);
